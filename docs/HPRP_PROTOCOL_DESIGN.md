...
- `text` 始终必填且必须是有效 UTF-8。图片模式下，图片来自当前 viewport 的 ANSI 读取，
  `text` 可以由紧随其后的独立纯文本读取生成；两者必须属于同一稳定目标，但不要求逐行或
  逐字一致；
...
图片请求渲染失败时，结果使用 `outcome: failed` 和 `terminal.image_failed`，并可以携带
`fallback_content`。`fallback_content` 必须来自图片读取后对同一稳定目标执行的独立纯文本
读取，不得改用其他 pane 或 replacement target。Server 可以只在通知场景使用该内容降级；
空白终端页也是合法结果，不能用 `text` 是否为空判断能否降级。用户主动执行图片终端命令
时仍应展示明确错误。
...
- 图片终端内容始终包含同一稳定目标的独立审计纯文本，不要求与图片逐行一致；
