...
- 在企业微信消息幂等去重后进入用户队列，再执行按用户滚动窗口限速，默认每秒 1 条、
  每分钟 20 条；队列满的输入单独记录 `queue_full` 审计结果。
...
- `/ls-reg` 先生成候选列表，Router 完整发送全部分段后才提交管理员独立内存快照；发送失败清除旧快照。
- `/apr`、`/rej` 先用同一位置的 `registration_id` 复核全局列表，再只使用稳定 ID 执行。
- 全局列表尾部新增不影响旧编号；删除或重排会整批拒绝，避免并发错批。
- 每次最多处理 10 项；审批尝试立即清除该管理员快照，批量单项失败不会阻止后续稳定 ID。
- 企业微信事件循环并发派发不同用户消息，现有 `UserExecutor` 继续保证同一用户严格串行。
- 单用户队列满时保留一个排在已接受任务之后的主动通知槽，避免同步过载回复阻塞事件循环。
