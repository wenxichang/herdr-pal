...
- 以 userid 为路由和隔离边界。
- 直接处理 `/userid`、`/ls`、独立的 `/N`/`/sel N` 和 `/help`。
- 将 `/N 内容`、`#N 内容` 的全局编号解析为稳定目标；前者在成功响应后切换，后者保持
  当前选择，二者都不额外执行 `/con`。
...
- 按 userid 维护双向滑动活跃时间；最近 2 分钟内其他会话的终端通知降级为简短提醒，避免
  多台机器的后台输出打断当前交互。命令后续分段始终完整发送。
...
快照；Hub 只校验并返回该引用，Router 再根据普通执行、`/N` 或 `#N` 的语义重绑、切换或
保持当前选择。
