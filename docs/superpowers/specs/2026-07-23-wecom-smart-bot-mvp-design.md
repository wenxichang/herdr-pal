...
单聊主动推送时，`aibot_send_msg.body.chatid` 使用配置中的
`allowed_user_id`，并显式设置 `chat_type: 1`。企业微信要求用户先在会话中给机器人发过
消息，才能向该单聊主动推送；未满足该平台前置条件时只报告投递失败，不缓存补发。
...
- 启动时先调用 `ping`，只接受与审计 Schema 完全一致的协议版本 `17`；其他版本进入
  `ProtocolMismatch`，不得继续读取或发送输入。升级 Schema 和协议版本必须单独审计。
