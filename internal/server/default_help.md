...
用户没有任何机器记录时，首台机器会直接收到本机 Key；已有机器时，申请会等待管理员审批。收到 Key 后发送 `/help`，按本帮助中的地址、下载和安装步骤部署 Herdr Pal。

【管理员审批】
以下命令只对服务端配置的注册审批管理员有效：
`/ls-reg` 列出待审批机器，完整送达后保存当前编号快照
`/apr 1 2 3` 批准指定编号，每次最多 10 项
`/rej 1 2 3` 驳回指定编号，每次最多 10 项

审批后编号快照立即失效，请重新执行 `/ls-reg` 核实当前条目顺序。
...
定向前缀不能用于 `/ls`、`/help`、`/reg`、管理员审批命令或另一个 `/N`。
