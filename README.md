...
字段缺失或空数组表示关闭企微审批命令，Web 管理审批仍可使用。新 pending 申请按数组顺序
...
`/rej 1 2 3` 驳回。编号只属于该管理员最近一次列表；任何审批尝试后快照立即失效，必须
...
| `/ls-reg` | 管理员：列出待审批机器并保存临时编号快照 |
| `/apr 1 2 3` | 管理员：批准指定编号，完成后必须重新 `/ls-reg` |
| `/rej 1 2 3` | 管理员：驳回指定编号，完成后必须重新 `/ls-reg` |
