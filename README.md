...
管理员从企业微信管理信息或组织内账户系统取得用户的企业微信 principal ID，并为每台接入
机器确定一个在该用户下不重复的机器标识，例如 `office-pc`。这些身份信息只在服务端签发
凭据时使用，终端用户无需写入 Herdr Pal 配置。管理员在服务端执行：
...
不能是 `/ls`、`/help` 或另一个 `/N`。`/N 内容` 只有在目标客户端
