...
```json
{
  "wecom": {
    "bot_id": "你的 Bot ID",
    "secret": "你的机器人 Secret",
    "registration_admin_ids": ["企业微信管理员用户 ID"]
  },
  "server": {
    "listen": "0.0.0.0:9443",
    "cert_file": "",
...
`wecom.registration_admin_ids` 配置允许在机器人内处理机器注册审批的企业微信用户 ID。
字段缺失或空数组表示关闭企微审批命令，`null` 和其他非数组类型会被拒绝；Web 管理审批仍可使用。新 pending 申请按数组顺序
轮转通知一位管理员；通知失败只记录日志，不重复通知下一位。所有配置管理员均可使用
`/ls-reg`、`/apr` 和 `/rej`。
...
当前单聊响应中只显示一次。如果用户已有机器，申请会进入待审批列表，可由 Web 管理台或
配置的企业微信管理员处理；批准后 Server 主动把 Key 发给申请人。审批发送失败时，新凭据
会被回滚，申请继续保留以便重试。收到 Key 后发送 `/help`，按管理员维护的安装地址和步骤
继续部署。

企业微信管理员先执行 `/ls-reg` 获取自己的临时编号快照，再执行 `/apr 1 2 3` 批准或
`/rej 1 2 3` 驳回，每次最多处理 10 项。编号只属于该管理员最近一次完整送达的列表；列表
任一分段发送失败时不会启用快照，任何审批尝试后快照立即失效，必须
重新执行 `/ls-reg`。多个管理员或 Web 后台并发处理导致编号位置变化时，Server 会整批拒绝
陈旧操作，不会把旧编号应用到其他申请。
...
| `/ls-reg` | 管理员：列出待审批机器，完整送达后保存临时编号快照 |
| `/apr 1 2 3` | 管理员：批准最多 10 个编号，完成后必须重新 `/ls-reg` |
| `/rej 1 2 3` | 管理员：驳回最多 10 个编号，完成后必须重新 `/ls-reg` |
...
不能是 `/ls`、`/help`、`/reg`、管理员审批命令或另一个 `/N`。`/N 内容` 只有在目标客户端
